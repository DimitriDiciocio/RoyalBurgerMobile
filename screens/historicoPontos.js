...
export default function HistoricoPontos({ navigation }) {
  ...
  const { userInfo, userData, isHeaderReady, addresses: cachedAddresses, loadingPoints } = useUserCache();
  ...
  const fetchLoyaltyHistory = async (userId) => {
    try {
      ...
      // Se não encontrou nenhum histórico, mantém o estado atual (não limpa)
      if (historyList.length === 0) {
        console.log('Nenhum histórico encontrado na resposta da API');
        setLoadingHistory(false);
        return; // Não limpa o histórico existente se a API retornar vazio
      }
      ...
      // Se após filtrar não tem nada, ainda assim define o histórico (pode ser que todos tenham 0 pontos)
      // Mas define como array vazio para mostrar mensagem apropriada
      if (filteredHistory.length === 0) {
        console.log('Nenhuma transação válida encontrada após filtrar');
        setHistory([]);
        setLoadingHistory(false);
        return;
      }
      ...
      if (orderIdsArray.length > 0) {
        try {
          ...
          // Se falhar ao buscar detalhes dos pedidos, não limpa o histórico
          // Apenas loga o erro e continua com o histórico sem detalhes completos
          console.warn('Erro ao buscar detalhes dos pedidos (não crítico):', error);
        }
      }
      ...
      console.log('Histórico enriquecido:', enrichedHistory.length, 'itens'); // Debug
      setHistory(enrichedHistory);
    } catch (error) {
      console.error('Erro ao buscar histórico de pontos:', error);
      // NÃO limpa o histórico se já existir - mantém o último estado válido
      // Só limpa se nunca foi carregado (history.length === 0)
      setHistory(prevHistory => {
        if (prevHistory.length === 0) {
          return [];
        }
        // Mantém o histórico anterior se já tinha dados
        console.warn('Mantendo histórico anterior devido a erro na atualização');
        return prevHistory;
      });
    } finally {
      setLoadingHistory(false);
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (ok && userData?.id) {
          ...
          await fetchLoyaltyHistory(userData.id);
        } else {
          setEnderecos([]);
        }
      } catch (e) {
    ...
    checkAuth();
  }, [isFocused, userData?.id, cachedAddresses.length]); // Usa userData?.id e cachedAddresses.length para evitar re-renders desnecessários
